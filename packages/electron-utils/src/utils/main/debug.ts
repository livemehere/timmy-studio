...
function toggleDevelopmentTools(
  win: BrowserWindow | null = BrowserWindow.getFocusedWindow()
): void {
  if (win) {
...
function shouldRun(options: DebugOptions): boolean {
  return options.isEnabled === true || (options.isEnabled === null && isDev());
}

function getOptionsForWindow(
  win: BrowserWindow,
...
function inspectElements(): void {
  ...
  const inspect = (): void => {
    (win as any)?.devToolsWebContents?.executeJavaScript(
