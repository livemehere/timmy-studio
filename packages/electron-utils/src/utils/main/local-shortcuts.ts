...
function _findShortcut(
  event: any,
  shortcutsOfWindow: ShortcutsCollection
): number {
  let i = 0;
  for (const shortcut of shortcutsOfWindow) {
...
const _onBeforeInput =
  (shortcutsOfWindow: ShortcutsCollection) => (e: any, input: Input) => {
    if (input.type === 'keyUp') {
      return;
    }
...
export function register(
  ...
  console.log(
    `Registering callback for ${accelerator} on window ${title(win)}`
  );
...
export function unregister(
  ...
  console.log(
    `Unregistering callback for ${accelerator} on window ${title(win)}`
  );
