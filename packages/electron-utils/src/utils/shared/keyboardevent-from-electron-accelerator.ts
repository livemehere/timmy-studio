 ...
 * @returns 업데이트된 상태
 */
function reduceKey({accelerator, event}: ReducerState, key: string): ReducerState {
 ...
 */
function toKeyEvent(accelerator: string): KeyboardEventState | {unsupportedKeyForPlatform: boolean} {
	let state: ReducerState | UnsupportedType = {accelerator, event: {}};
	while (state !== UNSUPPORTED && 'accelerator' in state && state.accelerator !== '') {
