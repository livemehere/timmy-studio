import { NumberField, ToggleField } from '../inputs';
import type { IEffect, EffectType } from '../../types/effect';
import type { IEffectMask } from '../../types/effect';
import { Button } from '@/components/ui/button';
import { Plus, Trash2 } from 'lucide-react';

interface EffectPropertyProps {
  effects: IEffect[];
  onChange: (effects: IEffect[]) => void;
  onChanged?: () => void;
}

export function EffectProperty({
  effects,
  onChange,
  onChanged,
}: EffectPropertyProps) {
  const handleChange = (newEffects: IEffect[]) => {
    onChange(newEffects);
    onChanged?.();
  };
  ...
  const handleAddEffect = (type: EffectType) => {
    ...
    handleChange([...effects, newEffect]);
  };

  const handleRemoveEffect = (id: string) => {
    handleChange(effects.filter((e) => e.id !== id));
  };

  const handleUpdateEffect = (id: string, updates: Partial<IEffect>) => {
    handleChange(effects.map((e) => (e.id === id ? { ...e, ...updates } : e)));
  };
          ...
          {effect.enabled && effect.type === 'blur' && (
            <div className="space-y-2 pt-2 border-t border-neutral-700">
              ...
              <NumberField
                ...
                onChange={(value) =>
                  handleUpdateParameter(effect.id, 'quality', Math.floor(value))
                }
                min={1}
                max={15}
                step={1}
